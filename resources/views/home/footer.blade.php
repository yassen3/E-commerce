<footer>
    <div class="container">
       <div class="row">
          <div class="col-md-4">
              <div class="full">
                 <div class="logo_footer">
                   <a href="{{asset('#')}}"><img width="210" src="/images/logo.png" alt="#" /></a>
                 </div>
                 <div class="information_f">
                   <p><strong>ADDRESS:</strong> 28 White tower, Street Name New York City, USA</p>
                   <p><strong>TELEPHONE:</strong> +91 987 654 3210</p>
                   <p><strong>EMAIL:</strong> yourmain@gmail.com</p>
                 </div>
              </div>
          </div>
          <div class="col-md-8">
             <div class="row">
             <div class="col-md-7">
                <div class="row">
                   <div class="col-md-6">
                <div class="widget_menu">
                   <h3>Menu</h3>
                   <ul>
                      <li><a href="{{asset('#')}}">Home</a></li>
                      <li><a href="{{asset('#')}}">About</a></li>
                      <li><a href="{{asset('#')}}">Services</a></li>
                      <li><a href="{{asset('#')}}">Testimonial</a></li>
                      <li><a href="{{asset('#')}}">Blog</a></li>
                      <li><a href="{{asset('#')}}">Contact</a></li>
                   </ul>
                </div>
             </div>
             <div class="col-md-6">
                <div class="widget_menu">
                   <h3>Account</h3>
                   <ul>
                      <li><a href="{{asset('#')}}">Account</a></li>
                      <li><a href="{{asset('#')}}">Checkout</a></li>
                      <li><a href="{{asset('#')}}">Login</a></li>
                      <li><a href="{{asset('#')}}">Register</a></li>
                      <li><a href="{{asset('#')}}">Shopping</a></li>
                      <li><a href="{{asset('#')}}">Widget</a></li>
                   </ul>
                </div>
             </div>
                </div>
             </div>
             <div class="col-md-5">
                <div class="widget_menu">
                   <h3>Newsletter</h3>
                   <div class="information_f">
                     <p>Subscribe by our newsletter and get update protidin.</p>
                   </div>
                   <div class="form_sub">
                      <form>
                         <fieldset>
                            <div class="field">
                               <input type="email" placeholder="Enter Your Mail" name="email" />
                               <input type="submit" value="Subscribe" />
                            </div>
                         </fieldset>
                      </form>
                   </div>
                </div>
             </div>
             </div>
          </div>
       </div>
    </div>
 </footer>
